use crate::types::{BdkInfo, BlockchainConfig, DatabaseConfig, DerivedKeyInfo, Entropy, ExtendedKeyInfo, KeyChainKind, Network, ResponseWallet, WordCount};
use crate::ffi::{AddressIndex, PartiallySignedBitcoinTransaction, Transaction, TxBuilder, Wallet, generate_mnemonic_from_word_count, generate_mnemonic_from_entropy, Balance, DescriptorSecretKey, DerivationPath, AddressAmount, get_extended_key_info, TransactionDetails, LocalUtxo};
use std::ops::Deref;
use std::str::FromStr;
...
use crate::utils::{config_bdk_network, config_blockchain, config_entropy, config_keychain_kind, config_network, config_word_count, handle_keys_error};

lazy_static! {
    static ref BDKINFO: RwLock<Option<BdkInfo>> = RwLock::new(None);
}
fn set_bdk_info(blockchain:Arc<AnyBlockchain>, wallet:Arc<Wallet>) {
...
pub fn import_wallet(json_wallet:String,
                   network: Network,
                     blockchain_config: BlockchainConfig,
                     database_config: DatabaseConfig) {
    let import = FullyNodedExport::from_str(&*json_wallet).unwrap();
    wallet_init(import.descriptor(), import.change_descriptor(), network, blockchain_config, database_config);
}
...
pub fn get_public_descriptor()->String
{
    let bdk_info = BDKINFO.read().unwrap().clone().unwrap();
    let wallet = bdk_info.wallet.unwrap();
    let res =  wallet.wallet_mutex.lock().unwrap().public_descriptor(KeychainKind::External).unwrap().unwrap().to_string();
    res
}
...
pub fn get_blockchain_hash(blockchain_height:u64)-> String
{
    let bdk_info = BDKINFO.read().unwrap().clone().unwrap();
    let blockchain = bdk_info.blockchain.unwrap();
   let x =  blockchain.get_block_hash(blockchain_height).unwrap().as_hash().to_string();
    x
}
pub fn sync_wallet()
{
...
pub fn get_new_internal_address() -> String
{
    let bdk_info = BDKINFO.read().unwrap().clone().unwrap();
    let wallet = bdk_info.wallet.unwrap();
    let address = wallet.get_internal_address(AddressIndex::New).unwrap().address;
    address
}
...
pub fn drain_wallet(recipient: String,  fee_rate: f32) -> String{
    ...
    let tx_builder = TxBuilder::new();
    let psbt = tx_builder
        .drain_wallet().drain_to(recipient)
        .fee_rate(fee_rate)
        .finish(&wallet);
    psbt.unwrap().serialize()
}
...
fn get_descriptor_secret_key(mnemonic:String, network:Network, password:Option<String>) -> DescriptorSecretKey {
...
mod tests {
    ...
    use crate::r_api::{broadcast, create_transaction, derive_dsk, export_wallet, generate_seed_from_entropy, generate_seed_from_word_count, get_balance, get_blockchain_hash, get_blockchain_height, get_descriptor_secret_key, get_new_address, get_transactions, parse_mnemonic, wallet_init};
    use crate::types::{BdkError, BlockchainConfig, DatabaseConfig, ElectrumConfig, Entropy, Network, WordCount};
    #[test]
    ...
    fn create_transaction_test() {
        ...
        let output_address = Address::from_script(
            &script,
            BdkNetwork::Testnet).unwrap().to_string();
    ...
    fn get_balance_test() {
        test_init_wallet();
        assert_eq!(get_balance().total, 10000);
    }
    #[test]
    fn generate_mnemonic_word_count_test(){
        let mnemonic= generate_seed_from_word_count(WordCount::WORDS15);
    ...
    #[test]
    fn unknown_word_test(){
        assert_eq!(
            parse_mnemonic(
                "letter advice cagex absurd amount doctor acoustic avoid letter advice cage above".to_string(),
            ),
            Err(BdkError::UnknownWord)
        );
    }

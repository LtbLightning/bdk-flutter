...
pub enum AddressParseError {
    ...
    #[error("witness version conversion/parsing error: {error_message}")] WitnessVersion {
        error_message: String,
    },

    #[error("witness program error: {error_message}")] WitnessProgram {
        error_message: String,
    },
...
pub enum Bip32Error {
    ...
    #[error("secp256k1 error: {error_message}")] Secp256k1 {
        error_message: String,
    },

    #[error("invalid child number: {child_number}")] InvalidChildNumber {
        child_number: u32,
    },
    ...
    #[error("unknown version: {version}")] UnknownVersion {
        version: String,
    },

    #[error("wrong extended key length: {length}")] WrongExtendedKeyLength {
        length: u32,
    },

    #[error("base58 error: {error_message}")] Base58 {
        error_message: String,
    },

    #[error("hexadecimal conversion error: {error_message}")] Hex {
        error_message: String,
    },

    #[error("invalid public key hex length: {length}")] InvalidPublicKeyHexLength {
        length: u32,
    },

    #[error("unknown error: {error_message}")] UnknownError {
        error_message: String,
    },
}
...
#[derive(Debug, thiserror::Error)]
pub enum Bip39Error {
    #[error("the word count {word_count} is not supported")] BadWordCount {
        word_count: u64,
    },
    ...
    #[error("entropy bit count {bit_count} is invalid")] BadEntropyBitCount {
        bit_count: u64,
    },
    ...
    #[error("ambiguous languages detected: {languages}")] AmbiguousLanguages {
        languages: String,
    },
}
...
pub enum CalculateFeeError {
    #[error("missing transaction output: {out_points:?}")] MissingTxOut {
        out_points: Vec<OutPoint>,
    },

    #[error("negative fee value: {amount}")] NegativeFee {
        amount: String,
    },
}
...
pub enum CannotConnectError {
    #[error("cannot include height: {height}")] Include {
        height: u32,
    },
}
...
pub enum CreateTxError {
    #[error("descriptor error: {error_message}")] Descriptor {
        error_message: String,
    },

    #[error("policy error: {error_message}")] Policy {
        error_message: String,
    },

    #[error("spending policy required for {kind}")] SpendingPolicyRequired {
        kind: String,
    },
    ...
    #[error("lock time conflict: requested {requested}, but required {required}")] LockTime {
        requested: String,
        required: String,
    },
    ...
    #[error("rbf sequence: {rbf}, csv sequence: {csv}")] RbfSequenceCsv {
        rbf: String,
        csv: String,
    },

    #[error("fee too low: required {required}")] FeeTooLow {
        required: String,
    },

    #[error("fee rate too low: {required}")] FeeRateTooLow {
        required: String,
    },
    ...
    #[error("output value below dust limit at index {index}")] OutputBelowDustLimit {
        index: u64,
    },
    ...
    #[error("coin selection failed: {error_message}")] CoinSelection {
        error_message: String,
    },
    ...
    #[error("psbt creation error: {error_message}")] Psbt {
        error_message: String,
    },

    #[error("missing key origin for: {key}")] MissingKeyOrigin {
        key: String,
    },

    #[error("reference to an unknown utxo: {outpoint}")] UnknownUtxo {
        outpoint: String,
    },

    #[error("missing non-witness utxo for outpoint: {outpoint}")] MissingNonWitnessUtxo {
        outpoint: String,
    },

    #[error("miniscript psbt error: {error_message}")] MiniscriptPsbt {
        error_message: String,
    },
}
...
pub enum CreateWithPersistError {
    #[error("sqlite persistence error: {error_message}")] Persist {
        error_message: String,
    },
    ...
    #[error("the loaded changeset cannot construct wallet: {error_message}")] Descriptor {
        error_message: String,
    },
}
...
pub enum DescriptorError {
    ...
    #[error("key error: {error_message}")] Key {
        error_message: String,
    },
    #[error("generic error: {error_message}")] Generic {
        error_message: String,
    },
    ...
    #[error("invalid descriptor character: {char}")] InvalidDescriptorCharacter {
        char: String,
    },

    #[error("bip32 error: {error_message}")] Bip32 {
        error_message: String,
    },
    ...
    #[error("key-related error: {error_message}")] Pk {
        error_message: String,
    },

    #[error("miniscript error: {error_message}")] Miniscript {
        error_message: String,
    },

    #[error("hex decoding error: {error_message}")] Hex {
        error_message: String,
    },
...
pub enum DescriptorKeyError {
    #[error("error parsing descriptor key: {error_message}")] Parse {
        error_message: String,
    },
    ...
    #[error("error bip 32 related: {error_message}")] Bip32 {
        error_message: String,
    },
}
...
pub enum ElectrumError {
    ...
    #[error("{error_message}")] Hex {
        error_message: String,
    },

    #[error("electrum server error: {error_message}")] Protocol {
        error_message: String,
    },

    #[error("{error_message}")] Bitcoin {
        error_message: String,
    },
    ...
    #[error(
        "error during the deserialization of a response from the server: {error_message}"
    )] InvalidResponse {
        error_message: String,
    },
    ...
    #[error("invalid domain name {domain} not matching SSL certificate")] InvalidDNSNameError {
        domain: String,
    },
    ...
    #[error("{error_message}")] SharedIOError {
        error_message: String,
    },
    ...
    #[error("{error_message}")] CouldNotCreateConnection {
        error_message: String,
    },
...
pub enum EsploraError {
    #[error("minreq error: {error_message}")] Minreq {
        error_message: String,
    },

    #[error("http error with status code {status} and message {error_message}")] HttpResponse {
        status: u16,
        error_message: String,
    },

    #[error("parsing error: {error_message}")] Parsing {
        error_message: String,
    },

    #[error("invalid status code, unable to convert to u16: {error_message}")] StatusCode {
        error_message: String,
    },

    #[error("bitcoin encoding error: {error_message}")] BitcoinEncoding {
        error_message: String,
    },

    #[error("invalid hex data returned: {error_message}")] HexToArray {
        error_message: String,
    },

    #[error("invalid hex data returned: {error_message}")] HexToBytes {
        error_message: String,
    },
    ...
    #[error("header height {height} not found")] HeaderHeightNotFound {
        height: u32,
    },
    ...
    #[error("invalid http header name: {name}")] InvalidHttpHeaderName {
        name: String,
    },

    #[error("invalid http header value: {value}")] InvalidHttpHeaderValue {
        value: String,
    },
...
pub enum ExtractTxError {
    #[error("an absurdly high fee rate of {fee_rate} sat/vbyte")] AbsurdFeeRate {
        fee_rate: u64,
    },
...
pub enum FromScriptError {
    ...
    #[error("witness version construction error: {error_message}")] WitnessVersion {
        error_message: String,
    },
...
pub enum LoadWithPersistError {
    ...
    #[error("the loaded changeset cannot construct wallet: {error_message}")] InvalidChangeSet {
        error_message: String,
    },
...
pub enum PersistenceError {
    #[error("writing to persistence error: {error_message}")] Write {
        error_message: String,
    },
}
...
pub enum PsbtError {
    ...
    #[error("invalid key: {key}")] InvalidKey {
        key: String,
    },
    ...
    #[error("duplicate key: {key}")] DuplicateKey {
        key: String,
    },
    ...
    #[error("non-standard sighash type: {sighash}")] NonStandardSighashType {
        sighash: u32,
    },

    #[error("invalid hash when parsing slice: {hash}")] InvalidHash {
        hash: String,
    },
    ...
    #[error("combine conflict: {xpub}")] CombineInconsistentKeySources {
        xpub: String,
    },

    #[error("bitcoin consensus encoding error: {encoding_error}")] ConsensusEncoding {
    ...
    #[error("invalid public key {error_message}")] InvalidPublicKey {
        error_message: String,
    },

    #[error("invalid secp256k1 public key: {secp256k1_error}")] InvalidSecp256k1PublicKey {
    ...
    #[error("invalid ECDSA signature: {error_message}")] InvalidEcdsaSignature {
        error_message: String,
    },

    #[error("invalid taproot signature: {error_message}")] InvalidTaprootSignature {
        error_message: String,
    },
    ...
    #[error("taproot tree error: {error_message}")] TapTree {
        error_message: String,
    },
    ...
    #[error("version error: {error_message}")] Version {
        error_message: String,
    },
    ...
    #[error("I/O error: {error_message}")] Io {
        error_message: String,
    },
...
pub enum PsbtParseError {
    #[error("error in internal psbt data structure: {error_message}")] PsbtEncoding {
        error_message: String,
    },

    #[error("error in psbt base64 encoding: {error_message}")] Base64Encoding {
        error_message: String,
    },
}
...
pub enum SignerError {
    ...
    #[error(
        "error while computing the hash to sign a P2WPKH input: {error_message}"
    )] SighashP2wpkh {
        error_message: String,
    },

    #[error(
        "error while computing the hash to sign a taproot input: {error_message}"
    )] SighashTaproot {
        error_message: String,
    },
    ...
    #[error("external error: {error_message}")] External {
        error_message: String,
    },

    #[error("Psbt error: {error_message}")] Psbt {
        error_message: String,
    },
}
...
pub enum SqliteError {
    #[error("sqlite error: {rusqlite_error}")] Sqlite {
...
pub enum TransactionError {
    ...
    #[error("invalid checksum: expected={expected} actual={actual}")] InvalidChecksum {
        expected: String,
        actual: String,
    },
    ...
    #[error("unsupported segwit version: {flag}")] UnsupportedSegwitFlag {
        flag: u8,
    },
...
pub enum TxidParseError {
    #[error("invalid txid: {txid}")] InvalidTxid {
        txid: String,
    },
}
...
impl From<BdkElectrumError> for ElectrumError {
    fn from(error: BdkElectrumError) -> Self {
        match error {
            BdkElectrumError::IOError(e) =>
                ElectrumError::IOError {
                    error_message: e.to_string(),
                },
            BdkElectrumError::JSON(e) =>
                ElectrumError::Json {
                    error_message: e.to_string(),
                },
            BdkElectrumError::Hex(e) =>
                ElectrumError::Hex {
                    error_message: e.to_string(),
                },
            BdkElectrumError::Protocol(e) =>
                ElectrumError::Protocol {
                    error_message: e.to_string(),
                },
            BdkElectrumError::Bitcoin(e) =>
            ...
            BdkElectrumError::InvalidResponse(e) =>
                ElectrumError::InvalidResponse {
                    error_message: e.to_string(),
                },
            BdkElectrumError::Message(e) =>
            ...
            BdkElectrumError::SharedIOError(e) =>
...
impl From<BdkParseError> for AddressParseError {
    fn from(error: BdkParseError) -> Self {
        match error {
            ...
            BdkParseError::WitnessVersion(e) =>
                AddressParseError::WitnessVersion {
                    error_message: e.to_string(),
                },
            BdkParseError::WitnessProgram(e) =>
...
impl From<BdkBip32Error> for Bip32Error {
    fn from(error: BdkBip32Error) -> Self {
        match error {
            ...
            BdkBip32Error::UnknownVersion(bytes) =>
...
impl From<BdkCalculateFeeError> for CalculateFeeError {
    fn from(error: BdkCalculateFeeError) -> Self {
        match error {
            ...
            BdkCalculateFeeError::NegativeFee(signed_amount) =>
...
impl From<BdkCreateTxError> for CreateTxError {
    fn from(error: BdkCreateTxError) -> Self {
        match error {
            BdkCreateTxError::Descriptor(e) =>
                CreateTxError::Descriptor {
                    error_message: e.to_string(),
                },
            BdkCreateTxError::Policy(e) =>
            ...
            BdkCreateTxError::LockTime { requested, required } =>
                CreateTxError::LockTime {
                    requested: requested.to_string(),
                    required: required.to_string(),
                },
            BdkCreateTxError::RbfSequence => CreateTxError::RbfSequence,
            BdkCreateTxError::RbfSequenceCsv { rbf, csv } =>
                CreateTxError::RbfSequenceCsv {
                    rbf: rbf.to_string(),
                    csv: csv.to_string(),
                },
            BdkCreateTxError::FeeTooLow { required } =>
                CreateTxError::FeeTooLow {
                    required: required.to_string(),
                },
            BdkCreateTxError::FeeRateTooLow { required } =>
                CreateTxError::FeeRateTooLow {
                    required: required.to_string(),
                },
            BdkCreateTxError::NoUtxosSelected => CreateTxError::NoUtxosSelected,
            BdkCreateTxError::OutputBelowDustLimit(index) =>
                CreateTxError::OutputBelowDustLimit {
                    index: index as u64,
                },
...
impl From<BdkCreateWithPersistError<chain::rusqlite::Error>> for CreateWithPersistError {
    fn from(error: BdkCreateWithPersistError<chain::rusqlite::Error>) -> Self {
        match error {
            BdkCreateWithPersistError::Persist(e) =>
                CreateWithPersistError::Persist {
                    error_message: e.to_string(),
                },
            BdkCreateWithPersistError::Descriptor(e) =>
...
impl From<BuildFeeBumpError> for CreateTxError {
    fn from(error: BuildFeeBumpError) -> Self {
        match error {
            BuildFeeBumpError::UnknownUtxo(outpoint) =>
                CreateTxError::UnknownUtxo {
                    outpoint: outpoint.to_string(),
                },
...
impl From<BdkDescriptorError> for DescriptorError {
    fn from(error: BdkDescriptorError) -> Self {
        match error {
            ...
            BdkDescriptorError::Key(e) =>
                DescriptorError::Key {
                    error_message: e.to_string(),
                },
            BdkDescriptorError::Policy(e) =>
            ...
            BdkDescriptorError::Bip32(e) =>
                DescriptorError::Bip32 {
                    error_message: e.to_string(),
                },
            BdkDescriptorError::Base58(e) =>
                DescriptorError::Base58 {
                    error_message: e.to_string(),
                },
            BdkDescriptorError::Pk(e) =>
                DescriptorError::Pk {
                    error_message: e.to_string(),
                },
            BdkDescriptorError::Miniscript(e) =>
                DescriptorError::Miniscript {
                    error_message: e.to_string(),
                },
            BdkDescriptorError::Hex(e) =>
                DescriptorError::Hex {
                    error_message: e.to_string(),
                },
...
impl From<KeyError> for DescriptorError {
    fn from(value: KeyError) -> Self {
        DescriptorError::Key { error_message: value.to_string() }
    }
}
...
impl From<BdkEsploraError> for EsploraError {
    fn from(error: BdkEsploraError) -> Self {
        match error {
            ...
            BdkEsploraError::HexToArray(e) =>
                EsploraError::HexToArray {
                    error_message: e.to_string(),
                },
            BdkEsploraError::HexToBytes(e) =>
...
impl From<BdkFromScriptError> for FromScriptError {
    fn from(error: BdkFromScriptError) -> Self {
        match error {
            ...
            BdkFromScriptError::WitnessProgram(e) =>
                FromScriptError::WitnessProgram {
                    error_message: e.to_string(),
                },
            BdkFromScriptError::WitnessVersion(e) =>
                FromScriptError::WitnessVersion {
                    error_message: e.to_string(),
                },
...
impl From<BdkLoadWithPersistError<chain::rusqlite::Error>> for LoadWithPersistError {
    fn from(error: BdkLoadWithPersistError<chain::rusqlite::Error>) -> Self {
        match error {
            BdkLoadWithPersistError::Persist(e) =>
                LoadWithPersistError::Persist {
                    error_message: e.to_string(),
                },
...
impl From<BdkPsbtError> for PsbtError {
    fn from(error: BdkPsbtError) -> Self {
        match error {
            ...
            BdkPsbtError::InvalidPublicKey(e) =>
                PsbtError::InvalidPublicKey {
                    error_message: e.to_string(),
                },
            BdkPsbtError::InvalidSecp256k1PublicKey(e) =>
            ...
            BdkPsbtError::InvalidEcdsaSignature(e) =>
                PsbtError::InvalidEcdsaSignature {
                    error_message: e.to_string(),
                },
            BdkPsbtError::InvalidTaprootSignature(e) =>
            ...
            BdkPsbtError::TapTree(e) =>
                PsbtError::TapTree {
                    error_message: e.to_string(),
                },
            BdkPsbtError::XPubKey(_) => PsbtError::XPubKey,
            BdkPsbtError::Version(e) =>
                PsbtError::Version {
                    error_message: e.to_string(),
                },
...
impl From<BdkPsbtParseError> for PsbtParseError {
    fn from(error: BdkPsbtParseError) -> Self {
        match error {
            BdkPsbtParseError::PsbtEncoding(e) =>
...
impl From<BdkSignerError> for SignerError {
    fn from(error: BdkSignerError) -> Self {
        match error {
            ...
            BdkSignerError::SighashTaproot(e) =>
                SignerError::SighashTaproot {
                    error_message: e.to_string(),
                },
            BdkSignerError::MiniscriptPsbt(e) =>
                SignerError::MiniscriptPsbt {
                    error_message: e.to_string(),
                },
            BdkSignerError::External(e) => SignerError::External { error_message: e },
            BdkSignerError::Psbt(e) =>
                SignerError::Psbt {
                    error_message: e.to_string(),
                },
        }
...
impl From<bdk_wallet::miniscript::Error> for DescriptorError {
    fn from(value: bdk_wallet::miniscript::Error) -> Self {
        DescriptorError::Miniscript { error_message: value.to_string() }
    }
}

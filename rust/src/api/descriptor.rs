...
impl FfiDescriptor {
    pub fn new(descriptor: String, network: Network) -> Result<Self, DescriptorError> {
        ...
        let (extended_descriptor, key_map) = descriptor.into_wallet_descriptor(
            &secp,
            network.into()
        )?;
        Ok(Self {
    ...
    pub fn new_bip44(
        ...
        match derivable_key {
            keys::DescriptorSecretKey::XPrv(descriptor_x_key) => {
                ...
                let (extended_descriptor, key_map, _) = Bip44(
                    derivable_key,
                    keychain_kind.into()
                ).build(network.into())?;
                Ok(Self {
            ...
            keys::DescriptorSecretKey::Single(_) =>
                Err(DescriptorError::Generic {
                    error_message: "Cannot derive from a single key".to_string(),
                }),
            keys::DescriptorSecretKey::MultiXPrv(_) =>
                Err(DescriptorError::Generic {
    ...
    pub fn new_bip44_public(
        ...
        let fingerprint = Fingerprint::from_str(fingerprint.as_str()).map_err(
            |e| DescriptorError::Generic { error_message: e.to_string() }
        )?;
        let derivable_key = &*public_key.ptr;
        match derivable_key {
            keys::DescriptorPublicKey::XPub(descriptor_x_key) => {
                let derivable_key = descriptor_x_key.xkey;
                let (extended_descriptor, key_map, _) = Bip44Public(
                    derivable_key,
                    fingerprint,
                    keychain_kind.into()
                ).build(network.into())?;
            ...
            keys::DescriptorPublicKey::Single(_) =>
                Err(DescriptorError::Generic {
                    error_message: "Cannot derive from a single key".to_string(),
                }),
            keys::DescriptorPublicKey::MultiXPub(_) =>
                Err(DescriptorError::Generic {
                    error_message: "Cannot derive from a multi key".to_string(),
                }),
        }
    ...
    pub fn new_bip49(
        ...
        match derivable_key {
            keys::DescriptorSecretKey::XPrv(descriptor_x_key) => {
                let derivable_key = descriptor_x_key.xkey;
                let (extended_descriptor, key_map, _) = Bip49(
                    derivable_key,
                    keychain_kind.into()
                ).build(network.into())?;
                Ok(Self {
    ...
    pub fn new_bip49_public(
        ...
        match derivable_key {
            keys::DescriptorPublicKey::XPub(descriptor_x_key) => {
                let derivable_key = descriptor_x_key.xkey;
                let (extended_descriptor, key_map, _) = Bip49Public(
                    derivable_key,
                    fingerprint,
                    keychain_kind.into()
                ).build(network.into())?;
    ...
    pub fn new_bip84(
        ...
        match derivable_key {
            keys::DescriptorSecretKey::XPrv(descriptor_x_key) => {
                let derivable_key = descriptor_x_key.xkey;
                let (extended_descriptor, key_map, _) = Bip84(
                    derivable_key,
                    keychain_kind.into()
                ).build(network.into())?;
                Ok(Self {
    ...
    pub fn new_bip84_public(
        ...
        match derivable_key {
            keys::DescriptorPublicKey::XPub(descriptor_x_key) => {
                let derivable_key = descriptor_x_key.xkey;
                let (extended_descriptor, key_map, _) = Bip84Public(
                    derivable_key,
                    fingerprint,
                    keychain_kind.into()
                ).build(network.into())?;
    ...
    pub fn new_bip86(
        ...
        match derivable_key {
            keys::DescriptorSecretKey::XPrv(descriptor_x_key) => {
                let derivable_key = descriptor_x_key.xkey;
                let (extended_descriptor, key_map, _) = Bip86(
                    derivable_key,
                    keychain_kind.into()
                ).build(network.into())?;
                Ok(Self {
    ...
    pub fn new_bip86_public(
        ...
        match derivable_key {
            keys::DescriptorPublicKey::XPub(descriptor_x_key) => {
                let derivable_key = descriptor_x_key.xkey;
                let (extended_descriptor, key_map, _) = Bip86Public(
                    derivable_key,
                    fingerprint,
                    keychain_kind.into()
                ).build(network.into())?;

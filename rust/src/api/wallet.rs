...
pub struct FfiWallet {
    pub opaque: RustOpaque<std::sync::Mutex<bdk_wallet::PersistedWallet<bdk_wallet::rusqlite::Connection>>>,
}
impl FfiWallet {
    pub fn new(
        ...
        let wallet: bdk_wallet::PersistedWallet<bdk_wallet::rusqlite::Connection> = bdk_wallet::Wallet
            ::create(descriptor, change_descriptor)
    ...
    pub fn load(
        ...
        let wallet: PersistedWallet<bdk_wallet::rusqlite::Connection> = bdk_wallet::Wallet
            ::load()
    ...
    pub fn reveal_next_address(opaque: FfiWallet, keychain_kind: KeychainKind) -> AddressInfo {
        opaque.get_wallet().reveal_next_address(keychain_kind.into()).into()
    }

    pub fn apply_update(&self, update: FfiUpdate) -> Result<(), CannotConnectError> {
        self.get_wallet().apply_update(update).map_err(CannotConnectError::from)
    }
    ...
    pub fn is_mine(&self, script: FfiScriptBuf) -> bool {
        self.get_wallet().is_mine(
            <FfiScriptBuf as Into<bdk_core::bitcoin::ScriptBuf>>::into(script)
        )
    }
    ...
    pub fn sign(
        ...
        opaque.get_wallet().sign(&mut psbt, sign_options.into()).map_err(SignerError::from)
    }

...
void main() {
  ...
  group('Generate Mnemonic', () {
    test('verify  word count', () async {
      ...
      when( mockBdkFlutter.generateMnemonic(wordCount: WordCount.WORDS12)).thenAnswer((_) async => 'puppy interest whip tonight dad never sudden response push zone pig patch');
      final res = await mockBdkFlutter.generateMnemonic(wordCount: WordCount.WORDS12);
      final wordCount = res.split(' ');
      expect(wordCount.length, 12);
    });
    test('verify generated mnemonic', () async {
      const mnemonic = "uncover melt orient double buyer birth run glad unhappy sport dizzy squeeze top offer axis rare bulk item";
      when(mockBdkFlutter.generateMnemonic(wordCount: WordCount.WORDS12)).thenAnswer((_) async => mnemonic);
      final res = await mockBdkFlutter.generateMnemonic(wordCount: WordCount.WORDS12);
      expect(mnemonic, res);
  ...
  group('Blockchain', () {
    ...
    test('verify create', () async {
      when( mockBlockchain.create(config: defaultConfig) ).thenAnswer((_) async => Blockchain());
    ...
    test('getHeight Exception', () async {
      try {
        when( mockBlockchain.getHeight()).thenThrow( const BlockchainException.unexpected("Blockchain not initialized"));
        await mockBlockchain.getHeight();
      } catch (error) {
        expect(error, const BlockchainException.unexpected("Blockchain not initialized"));
        expect(error, isA<BlockchainException>());
      }
    });

    test('verify getHash', () async {
      when( mockBlockchain.getBlockHash(any) ).thenAnswer((_) async => "0000000000004c01f2723acaa5e87467ebd2768cc5eadcf1ea0d0c4f1731efce");
      final res = await mockBlockchain.getBlockHash( 2396450) ;
      expect(res, "0000000000004c01f2723acaa5e87467ebd2768cc5eadcf1ea0d0c4f1731efce");
    });
    test('verify getHash  Exception', () async {
      try {
        when( mockBlockchain.getBlockHash(any)).thenThrow( const BlockchainException.unexpected("Blockchain not initialized"));
        await mockBlockchain.getHeight();
      } catch (error) {
        expect(error, const BlockchainException.unexpected("Blockchain not initialized"));
        expect(error, isA<BlockchainException>());
  ...
  group('Wallet', (){
    ...
    test('verify getAddress() Exception', () async  {
      try {
        when( mockWallet.getAddress(addressIndex: AddressIndex.New)).thenThrow( const WalletException.unexpected("Wallet not initialized"));
        await mockWallet.getAddress(addressIndex: AddressIndex.New);
      } catch (error) {
        expect(error, const WalletException.unexpected("Wallet not initialized"));
        expect(error, isA<WalletException>());
      }
    });
    test('Should return a new AddressInfo', () async {
      when( mockWallet.getAddress(addressIndex: AddressIndex.New)).thenAnswer((e) async => AddressInfo(index: 82, address: "tb1qzn0qsh9wdp0m7sx877p9u8kptnvmykm9ld5lyd"));
      final res = await mockWallet.getAddress(addressIndex: AddressIndex.New);
      expect(res.index, 82);
      expect(res.address, "tb1qzn0qsh9wdp0m7sx877p9u8kptnvmykm9ld5lyd");
    });
    test('Should return a last unused AddressInfo', () async {
      when( mockWallet.getAddress(addressIndex: AddressIndex.LastUnused)).thenAnswer((e) async => AddressInfo(index: 82, address: "tb1qzn0qsh9wdp0m7sx877p9u8kptnvmykm9ld5lyd"));
      final res = await mockWallet.getAddress(addressIndex: AddressIndex.LastUnused);
      expect(res.index, 82);
      expect(res.address, "tb1qzn0qsh9wdp0m7sx877p9u8kptnvmykm9ld5lyd");
    });
    ...
    test('verify getBalance() Exception', () async {
      try {
        when( mockWallet.getBalance()).thenThrow( const WalletException.unexpected("Wallet not initialized"));
    ...
    test('verify network() Exception', () async {
      try {
        when( mockWallet.network()).thenThrow( const WalletException.unexpected("Wallet not initialized"));
    ...
    test('verify listUnspent() Exception', () async {
      try {
        when( mockWallet.listUnspent()).thenThrow( const WalletException.unexpected("Wallet not initialized"));
  ...
  group('DescriptorSecret', () {
    test('verify derive()', () async {
      final path =  await mockDerivationPath.create(path: DEFAULT_DERIVATION_PATH);
  ...
  group('Tx Builder', () {
    test('Should return a TxBuilderException when funds are insufficient', () async {
      try {
        when(mockTxBuilder.finish(mockWallet)).thenThrow( const TxBuilderException.insufficientBroadcastAmount("InsufficientFunds { needed: 1252, available: 0 }"));
        await mockTxBuilder.finish(mockWallet);
      } catch (error) {
        expect(error, const TxBuilderException.insufficientBroadcastAmount("InsufficientFunds { needed: 1252, available: 0 }"));
        expect(error, isA<TxBuilderException>());
      }
    });
    test('Should return aTxBuilderException when no recipients are added', () async {
      try {
        when(mockTxBuilder.finish(mockWallet)).thenThrow( const TxBuilderException.unexpected("No Recipients Added"));
        await mockTxBuilder.finish(mockWallet);
      } catch (error) {
        expect(error, const TxBuilderException.unexpected("No Recipients Added"));
        expect(error, isA<TxBuilderException>());
      }
    });
    test('Verify addData() Exception', () async {
      try {
        when(mockTxBuilder.addData(data: List.empty())).thenThrow( const TxBuilderException.unexpected("List must not be empty"));
        await mockTxBuilder.addData(data: []);
      } catch (error) {
        expect(error, const TxBuilderException.unexpected("List must not be empty"));
        expect(error, isA<TxBuilderException>());
      }
    });
    test('Verify addRecipient() Exception', () async {
      try {
        when(mockTxBuilder.addRecipient(any, any)).thenThrow( const KeyException.unexpected("Script key not initialized yet"));
        mockTxBuilder.addRecipient(mockScript, 1200);
      } catch (error) {
        expect(error, const KeyException.unexpected("Script key not initialized yet"));
        expect(error, isA<KeyException>());
      }
    });
    test('Verify unSpendable()', () async {
      final res = mockTxBuilder.addUnSpendable(OutPoint(txid: "efc5d0e6ad6611f22b05d3c1fc8888c3552e8929a4231f2944447e4426f52056", vout: 1));
      expect(res, isNot(mockTxBuilder));
    });
    test('Verify necessary function are called for a  psbt transaction ', () async {
      ...
      verify(mockAddress.create(address: "2N4eQYCbKUHCCTUjBJeHcJp9ok6J2GZsTDt")).called(1);
    ...
    test('Create a proper psbt transaction ', () async {
      ...
      when(mockAddress.create(address: "2N4eQYCbKUHCCTUjBJeHcJp9ok6J2GZsTDt")).thenAnswer((_) async => mockAddress);
      when(mockAddress.scriptPubKey()).thenAnswer((_) async => mockScript);
      when( mockTxBuilder.addRecipient(mockScript, any)).thenReturn(mockTxBuilder);
      when(mockTxBuilder.finish(mockWallet)).thenAnswer((_) async => PartiallySignedBitcoinTransaction(psbtBase64:psbt));
      final address =  await mockAddress.create(address: "2N4eQYCbKUHCCTUjBJeHcJp9ok6J2GZsTDt");
  ...
  group('Bump Fee Tx Builder', () {
    test('Should return a TxBuilderException when txid is invalid', () async {
      try {
        when(mockBumpFeeTxBuilder.finish(mockWallet)).thenThrow( const TxBuilderException.invalidTxid("Invalid Txid"));
  ...
  group('Address', () {
    test('verify create()', () async {
      when( mockAddress.create(address: "tb1ql7w62elx9ucw4pj5lgw4l028hmuw80sndtntxt") ).thenAnswer((_) async => mockAddress);
      final res = await mockAddress.create(address: "tb1ql7w62elx9ucw4pj5lgw4l028hmuw80sndtntxt");
      expect(res,isA<Address>());
    });
    test('verify create() exception', () async {
      try {
        when( mockAddress.create(address: "tb1ql7w62elx9ucw4pj5lgw4l028hmuw80sndtntxt") ).thenThrow(const WalletException.invalidAddress("Invalid address"));
        await mockAddress.create(address: "tb1ql7w62elx9ucw4pj5lgw4l028hmuw80sndtntxt");
      } catch (error) {
  ...
  group('Derivation Path', () {
    test('verify create()', () async {
      when( mockDerivationPath.create(path: DEFAULT_DERIVATION_PATH) ).thenAnswer((_) async => mockDerivationPath);
      final res = await mockDerivationPath.create(path: DEFAULT_DERIVATION_PATH);
      expect(res,isA<DerivationPath>());
    });
    test('verify create() exception', () async {
      try {
        when( mockDerivationPath.create(path: DEFAULT_DERIVATION_PATH) ).thenThrow(const PathException.invalidPath("Invalid Derivation Path"));
        await mockDerivationPath.create(path: DEFAULT_DERIVATION_PATH);
      } catch (error) {
        expect(error, const PathException.invalidPath("Invalid Derivation Path"));

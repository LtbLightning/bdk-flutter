...
Future<void> main() async {
  ...
  //   return await secretKey.derive(path: await DerivationPath.create(path: path));
  // }
  ...
  group('Address', () {
    ...
    test('verify scriptPubKey ()', () async {
      ...
      expect(res.bytes, script);
    });
    test('verify scriptPubKey type()', () async {
      final res = await address.scriptPubkey();
      expect(res.bytes, isA<List<int>>());
    });
  ...
  group('Blockchain', () {
    ...
    test('verify getBlockHash() type', () async {
      ...
      final res = await blockchain.getBlockHash(height: height);
      expect(res, isA<String>());
    });
    test('verify estimateFee() type', () async {
      final res = await blockchain.estimateFee(target: 25);
  ...
  group('Mnemonic', () {
    test('verify mnemonic length; create()', () async {
      for (var e in [WordCount.words12, WordCount.words18, WordCount.words24]) {
        ...
        expect((await res.asString()).split(RegExp(r'\s+')).length, length);
      }
    });
